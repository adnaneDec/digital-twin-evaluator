...
# Function to generate GPT recommendations using OpenAI
def generate_recommendations(summary_data, comments):
    prompt = f"""
    Based on the following evaluation results, provide recommendations:
    - Scores: {summary_data.to_dict(orient='records')}
    - Comments: {comments}
    Ensure recommendations are actionable and relevant to improve compliance with digital twin standards.
    """
    response = client.chat.completions.create(model="gpt-4",
    messages=[
        {"role": "system", "content": "You are a helpful assistant knowledgeable about digital twin systems."},
...
def generate_chat_response(user_input, summary, comments):
    ...
    Provide a helpful and concise answer based on this context.
    """
    response = client.chat.completions.create(model="gpt-4",
    messages=[
        {"role": "system", "content": "You are a helpful assistant specialized in Digital Twin evaluation."},
        {"role": "user", "content": prompt}
    ],
    max_tokens=300)
    return response.choices[0].message.content.strip()
...
# Define the available pages in the sidebar
pages = ["Profile Identification"] + list(evaluation_framework.keys()) + ["Summary", "Chatbot"]

## Initialize navigation state if not set
if "navigation" not in st.session_state:
    st.session_state["navigation"] = "Profile Identification"

# If redirect flag for chatbot is set, update navigation to Chatbot
if "redirect_to_chatbot" not in st.session_state:
    st.session_state["redirect_to_chatbot"] = False

if st.session_state["redirect_to_chatbot"]:
    st.session_state["navigation"] = "Chatbot"
    st.session_state["redirect_to_chatbot"] = False  # Reset the flag

# Render sidebar with current navigation
page = st.sidebar.radio(
    "Select a Category",
    options=pages,
    index=pages.index(st.session_state["navigation"]),
    key="navigation_widget",  # Use a different key for the widget
)

# Update session state based on current selection
...
if page == "Profile Identification":
...
elif page in evaluation_framework:  # Ensure only framework pages are processed here
    ...
    if not valid:
        ...
        if st.button("Submit and Continue"):
            ...
            # Save the summary DataFrame to session state
            st.session_state["summary_df"] = summary_df

            st.session_state["navigation"] = "Chatbot"

            # Notify the user that they will be redirected
            st.write("Redirecting to the Chatbot page...")  


# On the "Chatbot" page
elif page == "Chatbot":
    # Set the header for the chatbot page
    st.header("Digital Twin Chatbot")

    # Check if the summary DataFrame is present in the session state
    if "summary_df" not in st.session_state:
        # If the summary is missing, display an error message and stop execution
        st.error("Summary data is missing. Please complete the evaluation first.")
        st.stop()

    # Retrieve the summary DataFrame from session state
    summary_df = st.session_state["summary_df"]
    # Generate a dictionary with average scores grouped by category
    summary = summary_df.groupby("Category")["Average Score"].mean().to_dict()
    # Retrieve the comments data from session state
    ...
    if "chat_history" not in st.session_state:
        ...
        # Generate an initial chatbot response using the summary and comments
        initial_response = generate_chat_response("How mature is my Digital Twin system?", summary, comments)
    ...
    for chat in st.session_state["chat_history"]:
        # Display user messages if available
        if chat["user"]:
    ...
    if user_input:
        # Generate a chatbot response based on the user input, summary, and comments
        bot_response = generate_chat_response(user_input, summary, comments)
        # Add the user query and bot response to the chat history
        st.session_state["chat_history"].append({"user": user_input, "bot": bot_response})
        # Instead of rerunning, update the `navigation` state to trigger updates
        st.session_state["navigation"] = "Chatbot"
