...
from openai import OpenAIError, RateLimitError  # To catch rate limit errors
import time
...
openai_api_key = st.secrets["openai"]["openai_api_key"] if st.secrets["openai"]["openai_api_key"] is not None else ""  # only for development environment, otherwise it should return None
client = OpenAI(api_key="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")

# Cache loading of FAISS vector store
# Define FAISS storage path inside `.streamlit/`
FAISS_PATH = ".streamlit/faiss_store"

@st.cache_resource
def load_faiss_vectorstore():
    """Loads FAISS vector store from `.streamlit/faiss_store` once per session."""
    try:
        if not os.path.exists(FAISS_PATH):
            st.warning("⚠️ FAISS vector store not found. Please ensure it's built and stored in `.streamlit/faiss_store`.")
            return None

        vectorstore = FAISS.load_local(
            FAISS_PATH, 
            OpenAIEmbeddings(), 
            allow_dangerous_deserialization=True  # Fix security issue
        )
...
# Load FAISS into session state exactly once
if "vectorstore" not in st.session_state:
    st.session_state["vectorstore"] = load_faiss_vectorstore()

# Catch RateLimitError in retrieval function
def answer_query_with_docs(query, num_results=5):
    """Retrieve relevant documents from FAISS and generate a GPT response, catching RateLimitError."""
    vectorstore = st.session_state.get("vectorstore")
    if not vectorstore:
        return "⚠️ FAISS vector store not loaded.", []

    # Step 1: Retrieve relevant documents (embedding the query)
    try:
        retriever = vectorstore.as_retriever(search_kwargs={"k": num_results})
        retrieved_docs = retriever.invoke(query)
    except RateLimitError:
        st.error("❌ You've hit your OpenAI embeddings rate limit or quota. Please try again later.")
        return "", []
    ...
    # Step 2: Extract document content and sources
    document_texts = "\n\n".join([doc.page_content for doc in retrieved_docs])
    pdf_sources = list({doc.metadata.get('source', 'Unknown') for doc in retrieved_docs})

    # Step 3: Use GPT to generate response with the retrieved context
    system_prompt = f"""
    You are a Digital Twin Evaluation Assistant. Answer the user's question based on the retrieved documents.
    If the documents do not contain enough information, state that explicitly.
    ...
    # Call ChatOpenAI as in your code
    llm = ChatOpenAI(model_name="gpt-4o-2024-08-06", temperature=0.2)
    try:
        response = llm(messages)
        return response.content, pdf_sources
    except RateLimitError:
        st.error("❌ You've hit your OpenAI Chat rate limit or quota. Please try again later.")
        return "", []
    except OpenAIError as e:
        st.error(f"❌ An OpenAI error occurred: {e}")
        return "", []
...
# initial prompt
def create_prompt(summary, user_input=None):
    """
    Generate a detailed GPT prompt with context for the evaluation summary.
    """
    base_prompt = f"""
    You are a Digital Twin Evaluation Assistant. Your task is to analyze and provide insights
    on the results of a Digital Twin Framework evaluation. The evaluation framework includes 
    the following categories, each with an average score:

    {summary}

    Please provide a detailed and helpful assessment.
    """
    if user_input:
...
evaluation_framework = {
    "Core Digital Twin Characteristics": {
        "description": "Digital Twins (DTs) are virtual representations of physical entities, enabling seamless bi-directional data exchange for real-time monitoring, simulation, and decision-making. The core DT components are the physical entity, the virtual copy and the data transfer linking them to one another.",
        "subcategories": [
            {
    ...
    "Connectivity and Synchronization": {
        "description": "A fundamental feature of DTs is their ability to maintain dynamic, bi-directional connections between physical and virtual entities. This involves ensuring synchronization through real-time or near-real-time data flows to support operational and strategic objectives, followed by the ability of the system to react and interact with the physical entity when needed (Whitin its application scope).",
        "subcategories": [
            {
        ...
        "subcategories": [
            ...
            },
            {
                ...
                "questions": [
                    ...
                    {"question": "How interpretable and explainable are the DT’s decisions?", "type": "fuzzy"}
                ]
            },
            {
...
if page == "Profile Identification":
    ...
    if not valid:
        ...
        if st.button("Submit and Continue"):
            ...
            # Inject initial question into chatbot session state
...
# Chatbot Page
    ...
    with st.sidebar:
        ...
        ## default_openai_api_key = "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"
    ...
    if openai_api_key == "" or openai_api_key is None or "sk-" not in openai_api_key:
        ...
        # Process the initial question automatically
        if len(st.session_state["messages"]) == 1 and st.session_state["messages"][0]["role"] == "user":
            initial_query = st.session_state["messages"][0]["content"][0]["text"]
                # Get FAISS-based response
            response_text, sources = answer_query_with_docs(initial_query)
        ...
        if prompt := st.chat_input("Hi! Ask me anything..."):
            ...
            # Get FAISS-based response
            response_text, sources = answer_query_with_docs(prompt)
            ...
            # Show document sources if found
            if sources:
                with st.expander("📚 Sources Used"):
                    for src in sources:
                        st.write(f"- {src}")
